...
void t_sp3Comp::compare(ostringstream& out) const {
  ...
  while (in1.currEpoch() && in2.currEpoch()) {
    ...
    if      (t1 < t2) {
      ...
      for (int i1 = 0; i1 < in1.currEpoch()->_sp3Sat.size(); i1++) {
        bncSP3::t_sp3Sat* sat1 = in1.currEpoch()->_sp3Sat[i1];	//第一个sp3的
        for (int i2 = 0; i2 < in2.currEpoch()->_sp3Sat.size(); i2++) {
          bncSP3::t_sp3Sat* sat2 = in2.currEpoch()->_sp3Sat[i2];		//第二个sp3的
          if (sat1->_prn == sat2->_prn) {
            epochOK        = true;
            epo->_dr[sat1->_prn]  = sat1->_xyz - sat2->_xyz;			// _dr两个sp3的坐标差
            epo->_xyz[sat1->_prn] = sat1->_xyz;							// xyz 第一个sp3的坐标
            if (sat1->_clkValid && sat2->_clkValid) {
              epo->_dc[sat1->_prn] = sat1->_clk - sat2->_clk;			// _dc两个sp3的钟差
            }
          }
        }
      }
  ...
  for (unsigned ie = 0; ie < epochs.size(); ie++) {
    ...
    t_epoch* epoch2 = 0;
    if (ie == 0) {
      epoch2 = epochs[ie+1];			// 后一个历元
    }
    else {
      epoch2 = epochs[ie-1];			// 前一个历元
    }
    double dt = epoch->_tt - epoch2->_tt;	//两个历元的时间差
    map<t_prn, ColumnVector>& dr   = epoch->_dr;		// sp3的xyz之差
    map<t_prn, ColumnVector>& xyz  = epoch->_xyz;		// 第一个sp3的xyz(当前的历元)
    map<t_prn, ColumnVector>& xyz2 = epoch2->_xyz;		// 第一个sp3的xyz(后一个历元)
    for (map<t_prn, ColumnVector>::const_iterator it = dr.begin(); it != dr.end(); it++) {
      const t_prn&  prn = it->first;
      if (xyz2.find(prn) != xyz2.end()) {
        const ColumnVector  dx = dr[prn];				// 两个sp3的坐标差（当前历元）
        const ColumnVector& x1 = xyz[prn];				
        const ColumnVector& x2 = xyz2[prn];
        ColumnVector vel = (x1 - x2) / dt;				// 第一个sp3的卫星速度(当前历元)
        XYZ_to_RSW(x1, vel, dx, dr[prn]);				// 第一个sp3的xyz(当前历元) // 速度 // 坐标差 // rsw
